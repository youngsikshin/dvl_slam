...
public:
    ...
    KeyframeDB(shared_ptr<ORBVocabulary> vocabulary);
    ~KeyframeDB();
...
//        keyframe->frame()->original_img().release();
        ...
        for(DBoW2::BowVector::const_iterator it= keyframe->BoW_vec().begin(); it!=keyframe->BoW_vec().end(); ++it)
            inverted_file_[it->first].push_back(keyframe);
    }
    ...
    vector< list<Keyframe::Ptr> >& inverted_file() { return inverted_file_; }
...
private:
    ...
    vector< list<Keyframe::Ptr> > inverted_file_;
};

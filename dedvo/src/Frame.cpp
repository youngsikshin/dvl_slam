...
//    :utime_(timestamp), camera_(camera), num_levels_(Config::cfg()->num_levels())
    :utime_(timestamp), camera_(camera), pointcloud_(pointcloud), num_levels_(Config::cfg()->num_levels()), max_level_(Config::cfg()->max_level())
{
    initialize_img(img);
    initialize_pc();
//    initialize_pc_OMP();
...
void Frame::Twc(Sophus::SE3f Twc)
{
...
//    cv::imshow("original_rgb",img_with_points);
    ...
    for(auto iter=pointcloud_.begin(); iter!=pointcloud_.end(); ++iter) {
      ...
      if(cnt % 4 > 0) continue;
        ...
        float v_min = 1.0;    float v_max = 15.0;    float dv = v_max - v_min;
        float v = xyz_ref(2);
        float r = 1.0; float g = 1.0; float b = 1.0;
        if (v < v_min)   v = v_min;
        if (v > v_max)   v = v_max;
...
void Frame::initialize_pc()
{
    ...
    for(auto iter=pointcloud_.begin(); iter!=pointcloud_.end(); ++iter) {
        ...
        if (camera_->is_in_image(uv, border, scale)) { // && iter->z < 5.0
...
//    cerr << "[Frame]\t Computation time of init_pc : " << (toc - tic)*1e-6 << endl;
